#!/usr/bin/env python3
"""
live_frustum_extractor_node.py
==============================
Fixes applied:
  1. Lowered MIN_LIDAR_PTS to 5 to protect distant/sparse pedestrians.
  2. Relaxed pedestrian dynamic depth margin to 1.2m to prevent limb amputation.
"""
...
class LiveFrustumExtractor(Node):
    ...
    MIN_LIDAR_PTS    = 5      # <--- Lowered from 10 to protect sparse pedestrians
    ...
    def __init__(self):
        ...
        self.get_logger().info('✅ LiveFrustumExtractor ready')
    ...
    def load_camera_calibration(self):
        ...
        if 'DistCoeff' in calib:
            self.dist = np.array(calib['DistCoeff']['data'], dtype=np.float64)
        else:
    ...
    def split_nearest_cluster(self, xyz: np.ndarray, intens: np.ndarray, gap_m: float = None) -> tuple:
        if gap_m is None:
            gap_m = self.OCCLUSION_GAP_M
        if len(xyz) < 2:
            return xyz, intens
    ...
    def process_frame(self):
        ...
        for i, detection in enumerate(self.latest_bbox_msg.detections):
            ...
            # ── DYNAMIC DEPTH MARGIN ─────────────────────────────────────
            if fusion_class == 1:       # Pedestrian
                dynamic_margin = 1.2    # <--- Relaxed slightly to prevent noise amputation
            ...
            if y_span < self.MIN_HEIGHT_SPAN:
                continue

            # Primary depth gap split 
            box_l_xyz, box_l_intens = self.split_nearest_cluster(box_l_xyz, box_l_intens)

            if len(box_l_xyz) < self.MIN_LIDAR_PTS:
                continue

            # Foreground depth filter (Applies strict semantic boundary)
        ...
        if frustum_count > 0:
            self.get_logger().debug(f'Published {frustum_count} frustum(s) this frame')
